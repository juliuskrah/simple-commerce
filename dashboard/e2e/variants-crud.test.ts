		...
		await page.evaluate(
			async (vars) => {
				await fetch('/graphql', {
					...
					body: JSON.stringify({
						operationName: 'AddProductVariant',
						query: `mutation AddProductVariant($productId: ID!, $input: ProductVariantInput!) { addProductVariant(productId: $productId, input: $input) { id title sku price { amount currency } } }`,
						variables: vars
					})
				});
			},
			{
				productId: 'prod_1',
				input: { title: 'E2E Variant', sku, price: { amount: 123.45, currency: 'USD' } }
			}
		);
		...
		const variantId = await page.evaluate(async () => {
			const res = await fetch('/graphql', {
				...
				body: JSON.stringify({
					...
					query: `query ProductDetail($id: ID!) { product(id: $id) { variants(first:20){ edges { node { id sku } } } } }`,
					variables: { id: 'prod_1' }
				})
			});
		...
		await page.evaluate(
			async (vars) => {
				await fetch('/graphql', {
					...
					headers: { 'content-type': 'application/json' },
					body: JSON.stringify({
						operationName: 'UpdateProductVariant',
						query: `mutation UpdateProductVariant($id: ID!, $input: ProductVariantInput!) { updateProductVariant(id:$id,input:$input){ id title sku } }`,
						variables: vars
					})
				});
			},
			{
				id: variantId,
				input: { title: newTitle, sku: 'KEEP-SKU', price: { amount: 222.22, currency: 'USD' } }
			}
		);
	...
	test('delete variant (mocked)', async ({ page }) => {
		...
		await page.evaluate(async (id) => {
			await fetch('/graphql', {
				...
				headers: { 'content-type': 'application/json' },
				body: JSON.stringify({
					operationName: 'DeleteProductVariant',
					query: `mutation DeleteProductVariant($id: ID!) { deleteProductVariant(id:$id) }`,
					variables: { id }
				})
			});
		}, variantId);
