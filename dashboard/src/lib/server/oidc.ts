...
export async function createAuthorizationUrl(request: Request) {
	...
	const client = new OAuth2Client(OIDC_CLIENT_ID, null, redirectUri);

	// Create authorization URL with the required scopes
	const url = client.createAuthorizationURL(config.authorization_endpoint, state, [
		'openid',
		'profile',
		'email'
	]);
...
export async function validateAuthorizationCode(code: string, request: Request) {
	...
	try {
		const tokens = await client.validateAuthorizationCode(config.token_endpoint, code, null);
